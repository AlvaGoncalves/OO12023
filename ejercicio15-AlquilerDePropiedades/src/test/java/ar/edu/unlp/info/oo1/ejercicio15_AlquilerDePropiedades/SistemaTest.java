...
class SistemaDeAlquileresTest {
	...
	void testListarPropiedadesDisponibles() {
		assertEquals(2,this.sistema.buscarPropiedadesDisponibles(lapso).size());
		this.propiedad1.agregarReserva(reserva1);
		assertEquals(1,this.sistema.buscarPropiedadesDisponibles(lapso).size());
	}
	...
	void testEliminarReserva() {
		...
		assertEquals(0,this.sistema.buscarPropiedadesDisponibles(lapso).size());
		assertEquals(0,this.sistema.buscarPropiedadesDisponibles(lapso2).size());
		assertEquals(1,this.sistema.buscarPropiedadesDisponibles(lapso3).size());
		this.sistema.eliminarReserva(reserva1);
		this.sistema.eliminarReserva(reserva2);
		this.sistema.eliminarReserva(reserva3);
		assertEquals(0,this.sistema.buscarPropiedadesDisponibles(lapso).size());
		assertEquals(0,this.sistema.buscarPropiedadesDisponibles(lapso2).size());
		assertEquals(2,this.sistema.buscarPropiedadesDisponibles(lapso3).size());
	}
